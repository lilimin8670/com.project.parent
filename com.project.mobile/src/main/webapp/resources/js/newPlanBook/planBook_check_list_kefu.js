...
function goBack() {
    if (from) {
        ...
        window.location.href=CP+"/service/homepage/initSecondKefuPage?code=m-kf-6&title=计划书审核";
    }
}
...
function showPlanBook(id, approveStat) {
    ...
    if (approveStat == "待审批") {
        window.location.href = CP + "/service/kfPlanBook/getCheckView?id=" + id + "&nameType=1&btnType=0";
    } else if (approveStat == "审核通过" || approveStat == "驳回") {
        window.location.href = CP + "/service/kfPlanBook/getCheckView?id=" + id + "&nameType=1&btnType=1";
    }
}
/**
 * 查看计划书审批进度
