 ...
 * @param type
 */
function familyPopTips(id, type) {
    layer.msg(type, {time: 3000});
    $("#anchor_scroll").attr("href", "input[name='" + id+"']");
    $("input[name='" + id+"']").focus();
    document.getElementById("anchor_scroll").click();
}
...
function vilidate() {
    ...
    if (form1 == "block") {
        ...
        $("#family_info li").each(function () {
            ...
            console.log("name:"+name,"value:"+value,"selecterId:"+selecterId);
            //判断控件的名称是否存在
            if (name){
                if (value.trim() == ""){
                    ...
                    console.log("arrivalTimeStr:"+arrivalTimeStr,"startTimeStr:"+value);
                    ...
                    console.log("arrivalTimeStr:"+value,"startTimeStr:"+startTimeStr);
 ...
 * @returns {boolean}
 */
function verifyCompanyPhone(phone, name) {
    var regex = /^((\+)?86|((\+)?86)?)0?1[34578]\d{9}$/;
    if (phone.match(regex)) {
        return true;
    } else {
        familyPopTips(name, "手机号不正确");
        return false;
    }
}
