...
function loadData() {
    $.ajax({
        url: basePath + "/service/storeTrace/getStoreTraceRecordsList?time=" + (new Date()).valueOf(),
        data: {
 ...
 */
function add() {
    window.location.href = basePath + "/service/storeTrace/initAddTrackRecord?trackRecordsId=" + trackRecordsId + "&type=" + type + "&time=" + (new Date()).valueOf();
};
