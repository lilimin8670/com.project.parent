            ...
            if (result.data!=0) {
                ...
                $.post(cp+"/service/check/registerForm/checkRegisterForm",param,function(result){
                    ...
                    if(result.success) {
                        ...
                        window.location.href = cp + "/service/check/registerForm/returnRegisterFormCheckTypeList?type="+type+"&classify="+classify+"&isCheckFlag=1"+"&from="+from +"&processId="+ processId;
                    }else {
...
function fanhui(){
    if (actionFrom == "check"){//报名表审核
        window.location.href = cp+"/service/check/registerForm/returnRegisterFormCheckTypeList?type="+type+"&classify="+classify+"&from="+from;
    }else if (actionFrom == "manger"){//客服版的报名表管理
        window.location.href = cp+"/service/dj/registerForm/returnKefuRegisterFormManager?classify="+classify;
    ...
    $.post(cp + "/service/check/registerForm/checkRegisterForm", param, function (result) {
        ...
        if (result.success) {
            if(isEdit == "noEdit"){
                ...
                window.location.href = cp + "/service/strokeOrder/returnOrderAddOrEdit?regFormId=" + regFormId + "&type=" + type + "&memberId=" + memberId + "&storeName=" + storeName + "&from=" + from + "&regProStateId=" + regProStateId + "&classify=" + classify + "&processId=" + processId + "&doType=" + doType;
            }
