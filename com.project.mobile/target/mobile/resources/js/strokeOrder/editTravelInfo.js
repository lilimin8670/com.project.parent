...
var flagAarray = new Array();
var cardAarray = new Array();
...
function vilidate() {
    ...
    var pt_sex = $("#pt_sex").val();
    var pt_phone = $("#pt_phone").val();
    var pt_IdNumber = $("#pt_IdNumber").val();
    var pt_xctype = $("#pt_xctype").val();
    var pt_hbcs = $("#pt_hbcs").val();
    var pt_qctime = $("#pt_qctime").val();
    var pt_ddtime = $("#pt_ddtime").val();
    var pt_iscohabit = $("#pt_iscohabit").val();
    // 	入院信息
...
function submitVaild(ry_heart) {
    if (ry_heart.trim() == "") {
        popTips("ry_heart", "入院中心为必填项");
        return false;
    } else {
        var strokeId = $("#strokeId").val();
        var url = basePath + "/service/strokeOrder/queryStatus";
        $.post(url, {id: strokeId}, function (res) {
            if (res.success == false) {
                noty({
                    text: "此行程已被编辑等待审核，不能再提交了!", timeout: 1000, callback: {
                        afterClose: function () {
