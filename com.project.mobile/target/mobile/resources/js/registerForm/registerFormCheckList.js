$(function(){
    $(".one header").css({"z-index":10000}).show();
...
function showDetail(id,regProStateId,processId,doType){
    if (status == "1"){
        window.location.href = cp + "/service/check/registerForm/returnRegisterFormCheckDetail?regFormId="+id+"&type="+type+"&classify="+classify+
            "&processId="+processId+"&regProStateId="+regProStateId+"&doType="+doType+"&from="+from+"&actionFrom=check";
    }
}
/**
 * 审核
        ...
        success:function(result){
            if (result.success){
                ...
                if (registerForm.status == 0 || registerForm.status == 4){
                    ...
                    window.location.href = cp + "/service/check/registerForm/returnRegisterFormCheck?regFormId="+id+"&type="+type+"&classify="+classify+"&processId="+processId+"&regProStateId="+regProStateId+"&from="+from+"&actionFrom=check";
                }
            }else {
...
function fanhui(){
    if (from=="push"){
        ...
        window.location.href = cp + "/service/check/registerForm/returnRegisterFormCheckType?classify="+classify+"&from="+from;
    }
}
