...
function addPlanBook(type){
    ...
    if(resp=="true"){
        window.location.href = CP + "/service/kfPlanBook/addPlanBook?type="+type;
    }else{
...
function showPlanBook(id){
    window.location.href = CP + "/service/kfPlanBook/editPlanBook?id="+id+"&type="+type+"&btnType=1";
}
/**
 * 删除样式处理
 */
...
function schedulePlanBook(planBookId){
    ...
    $.post(url, param, function (result) {
        if(result.success) {
            ...
            $("#processIframe").attr('src',CP+"/service/kfPlanBook/getView?type="+type+"&url=/planBook/processprogress");
...
function editPlanBook(id){
    window.location.href = CP + "/service/kfPlanBook/editPlanBook?id="+id+"&type="+type;
}
