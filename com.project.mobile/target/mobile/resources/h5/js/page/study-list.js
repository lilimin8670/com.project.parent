...
function init() {
    ...
    $.ajax({
        url: '/study-list',              //请求的url地址，这是相对于现在的位置的地址
        ...
        success: function (response) { //请求成功时处理
            ...
            if (response) {
                if (response.status == 0) {
                    var studyList=response.data;
                    if(studyList.studyList.length){
                        $("#studyList").append(template("studyListTemplate",studyList));
                        ++pageNumber;
                    }
...
function openstudyDetail(studyId){
    window.location.href="/h5/study-detail.html?sessionid="+sessionId+"&userAgent="+userAgent+"&studyId="+studyId+"&shareOrNot=1";
}
